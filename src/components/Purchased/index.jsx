...
function Purchased(props) {
  ...
  const PlayNPause = async (purchaseId, Paused) => {
    ...
    await fetch(BASE_URL + "subscription/pause_resume/", {
      ...
      body: JSON.stringify({ user: token.user.pk,  purchased_id: purchaseId, is_paused: pausedStatus }),
    })
      .then((response) => response.json())
      .then((result) => {
        if (result.code == "token_not_valid") {
                    ...
                    <Grid container spacing={3}>
                      ...
                      >
                        ...
                        <Grid item xs={2} className="grid-flex">
                          ...
                          >
                            <SwitchUnstyled component={Root} {...label} defaultChecked = {item.is_paused ? false: true} onClick={()=>PlayNPause(item.plan.id, item.is_paused)}/>
                          </Typography>
